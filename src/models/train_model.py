...
class Trainer:
    def __init__(
        ...
        self.log_dir = datatools.dir_add_timestamp(settings.logdir)
    ...
    def trainbatches(self) -> float:
        ...
        for _ in tqdm(range(train_steps), colour="#1e4706"):
            ...
            x, y = next(iter(self.traindataloader.stream()))
    ...
    def evalbatches(self) -> Tuple[Dict[str, float], float]:
        ...
        for _ in range(valid_steps):
            x, y = next(iter(self.validdataloader.stream()))
            yhat = self.model(x)
...
def trainloop(
    ...
    if "tensorboard" in tunewriter:
        log_dir = datatools.dir_add_timestamp(log_dir)
